...
public class ALG_Databank extends JFrame {
    ...
    private JPanel createAddressForm() {
        ...
        // Unten eine Button-Leiste für Speichern UND Excel-Export
        JPanel actionButtonPanel = new JPanel(new GridLayout(1, 2, 5, 5));
        JButton btnSave = new JButton("Änderungen speichern");
        JButton btnExcel = new JButton("Excel-Export (.csv / .xlsx)");

        btnSave.addActionListener(e -> saveAddressChanges());
        btnExcel.addActionListener(e -> excelExportAusfuehren()); // Hier wird Ihre Prozedur aufgerufen!

        actionButtonPanel.add(btnSave);
        actionButtonPanel.add(btnExcel);
    ...
    private void excelExportAusfuehren() {
        ...
        // Filter auf Excel-konforme CSV-Dateien anpassen, da wir ohne Apache POI arbeiten
        fileChooser.setFileFilter(new javax.swing.filechooser.FileNameExtensionFilter("Excel-kompatible CSV (*.csv)", "csv"));
        fileChooser.setSelectedFile(new File("Adressen_Export.csv"));
        ...
        if (fileChooser.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
            ...
            // Direkter Export der addressList in die Datei f
            try (FileOutputStream os = new FileOutputStream(f);
                 BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(os, "UTF-8"))) {

                // 1. UTF-8 BOM schreiben, damit Excel deutsche Umlaute direkt versteht
                os.write(new byte[]{(byte)0xEF, (byte)0xBB, (byte)0xBF});

                // 2. Excel anweisen, das Semikolon als Trenner zu nutzen
                writer.write("sep=;\n");

                // 3. Spaltenüberschriften
                writer.write("ID;Vorname;Nachname;Straße;PLZ;Stadt;E-Mail\n");

                // 4. Datenzeilen aus Ihrer addressList schreiben
                for (Address addr : addressList) {
                    writer.write(String.format("%s;%s;%s;%s;%s;%s;%s\n",
                            addr.getId() != null ? addr.getId() : "",
                            addr.getFirstName() != null ? addr.getFirstName() : "",
                            addr.getLastName() != null ? addr.getLastName() : "",
                            addr.getStreet() != null ? addr.getStreet() : "",
                            addr.getZipCode() != null ? addr.getZipCode() : "",
                            addr.getCity() != null ? addr.getCity() : "",
                            addr.getEmail() != null ? addr.getEmail() : ""
                    ));
                }

                JOptionPane.showMessageDialog(this, "Excel-Export erfolgreich abgeschlossen!");
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(this, "Fehler beim Excel-Export: " + ex.getMessage(), "Fehler", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
}